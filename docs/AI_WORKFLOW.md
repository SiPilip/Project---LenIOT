...
| 2026-09-19 | Fix: MapLibre Web Worker Bundling in Vite | Mengatasi permasalahan klaster dan heatmap yang tidak muncul di browser akibat kegagalan resolusi Web Worker MapLibre di lingkungan Vite. Menambahkan import workerUrl `?worker&url` dengan `setWorkerUrl()`, melengkapi `vite-env.d.ts`, memperbarui fontstack ke `Noto Sans Bold`/`Regular` yang valid, dan memastikan re-sinkronisasi data saat pergantian mode | Konfirmasi klaster dan heatmap tampil dengan sempurna |
