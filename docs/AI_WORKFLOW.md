...
| 2026-09-19 | Reusable PinpointMarker Component & Cluster Sync | Ekstraksi komponen pin penanda ke komponen reusable `PinpointMarker.tsx` lengkap dengan unit tests, serta sinkronisasi penanda DOM unclustered pada mode Clusters agar identik 1:1 dengan mode Pinpoints (kepala lingkaran hijau brand, inner disc ikon, manik status, jarum penunjuk, dan badge kartu putih label nama) | Konfirmasi tampilan penanda unclustered di mode Clusters sama persis dengan mode Pinpoints |
| 2026-09-19 | Density Heatmap Recalibration & Gradient Smoothing | Kalibrasi ulang parameter Density Heatmap agar sesuai presisi dengan legenda: menurunkan heatmap-weight ke 1.0, menyeimbangkan heatmap-intensity (0.6 - 1.3), memperhalus radius sebaran (14px - 56px), menurunkan opacity ke 0.75 agar peta jalan tetap terbaca, dan menerapkan 7-stop gradasi hijau ColorBrewer alami (mint lembut s/d emerald tua) untuk mengeliminasi noda gelap pekat | Verifikasi sebaran heatmap halus, transparan, dan sesuai gradasi legenda |
| 2026-09-19 | React + Vite Best Practices Optimization (/react-vite-best-practices) | Mengeliminasi dependensi `react-dom/server` dari bundle browser di `PinpointMarker.tsx`, mengimplementasikan rekonsiliasi DOM marker in-place (`updatePinpointElement`) di `MapView.tsx` guna mencegah thrashing DOM/marker recreation saat seleksi entitas, serta mengonfigurasi `manualChunks` di `vite.config.ts` untuk memisahkan `vendor-maplibre`, `vendor-react`, dan `vendor-tanstack`, memangkas ukuran `index.js` sebesar 90% (1,770 kB -> 181 kB) dan meluluskan `.\scripts\check.ps1` 100% green | Konfirmasi build bebas warning, chunking vendor optimal, dan interaksi peta bebas thrashing |
